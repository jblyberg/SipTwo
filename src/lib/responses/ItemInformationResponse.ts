...
export class ItemInformationResponse extends ResponseMessage {
  parse(message: string): IItemInformationResponse {
    ...
    const data: IItemInformationResponse = {
      circulationStatus: CirculationStatus.parse(this.message.slice(2, 4)) || 'UNKNOWN',
      securityMarker: SecurityMarker.parse(this.message.slice(4, 6)) || 'UNKNOWN',
      feeType: FeeType.parse(this.message.slice(6, 8)) || 'UNKNOWN',
